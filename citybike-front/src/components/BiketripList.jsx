...
const BiketripList = () => {
  ...
  const fetchBiketrips = async () => {
    ...
    const response = await axios.get(
      `http://192.168.1.130:3001/api/biketrips?page=${0}&search=${searchValue}&order=${sortBy}`
    )
  ...
  const fetchMore = async () => {
    setFetching(true)
    const response = await axios.get(
      `http://192.168.1.130:3001/api/biketrips?page=${page}&search=${searchValue}&order=${sortBy}`
    )
